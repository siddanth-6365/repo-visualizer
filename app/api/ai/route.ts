...
export async function POST(request: Request) {
  try {
    ...
    // 1. Truncate inputs to control context size
    const treeLines =
      fileTree
...
`;
    ...
    const explanationMatch = explanationRaw.match(/<explanation>([\s\S]*?)<\/explanation>/);
    const explanationText = explanationMatch
      ? explanationMatch[1].trim()
      : explanationRaw.trim();

    // 3️⃣ STEP 2: Component mapping
    const system2 = 'You are a software architect assistant. Produce a mapping wrapped in <component_mapping> tags.';
    const user2 = `
<explanation>
${explanationText}
</explanation>
...
`;
    ...
    const mappingMatch = mappingRaw.match(/<component_mapping>([\s\S]*?)<\/component_mapping>/);
